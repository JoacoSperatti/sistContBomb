...
export default function CargarClientes() {
  ...
  // Eliminamos nroRifa de acá
  const estadoInicialForm = {
    cliente: '', vendedor: '',
    correo: '', campana: '2025-2026', telefono: '',
    domicilio: '', metodoPago: '', esAbonado: false
  };
  ...
  const [pagos, setPagos] = useState(
    meses.reduce((acc, mes) => ({ ...acc, [mes]: { pagado: false, nroRifa: '', metodoPago: '' } }), {})
  );
  ...
  const handlePagoChange = async (mes) => {
    ...
    if (pagoActual && pagoActual.pagado) {
      setPagos({ ...pagos, [mes]: { pagado: false, nroRifa: '', metodoPago: '' } });
    } else {
      const { value: formValues } = await Swal.fire({
        title: `Registrar pago de ${mes}`,
        html: `
          <div class="text-left font-sans">
            <label class="block text-sm font-bold text-gray-700 mb-1 mt-4">Nro. de Rifa para este mes:</label>
        ...
        preConfirm: () => {
          return {
            nroRifa: document.getElementById('swal-nro').value,
      ...
      if (formValues) {
        setPagos({
          ...pagos,
          [mes]: { pagado: true, nroRifa: formValues.nroRifa, metodoPago: formValues.metodoPago }
        });
      }
    }
  };
  ...
  const handleGuardar = async (e) => {
    ...
    try {
      ...
      setPagos(meses.reduce((acc, mes) => ({ ...acc, [mes]: { pagado: false, nroRifa: '', metodoPago: '' } }), {}));
    ...
    <div className="min-h-screen bg-gray-100 p-8 font-sans">
      ...
      <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
        <form onSubmit={handleGuardar} className="p-8">
          ...
          <div className="mt-10">
            <h2 className="text-xl font-bold text-blue-800 mb-4 border-b pb-2">Control de Cuotas (Detallar número por mes)</h2>
                  ...
                  <div key={mes} onClick={() => handlePagoChange(mes)} className={`cursor-pointer p-3 rounded-xl border-2 transition-all flex flex-col items-center justify-center gap-1 select-none ${estaPagado ? 'bg-green-50 border-green-500 shadow-inner' : 'bg-white border-gray-200 hover:border-blue-300 hover:bg-blue-50'}`}>
                    ...
                    {estaPagado ? (
                      <>
                        ...
                        {pagos[mes].nroRifa && <span className="text-[11px] font-bold bg-green-200 text-green-800 px-2 py-0.5 rounded-md">#{pagos[mes].nroRifa}</span>}
                        {pagos[mes].metodoPago && <span className="text-[10px] text-gray-600 uppercase font-bold">{pagos[mes].metodoPago}</span>}
                      </>
                    ) : (
          ...
          <div className="mt-10 flex justify-end">
            <button type="submit" disabled={cargando} className={`font-bold py-3 px-8 rounded-xl shadow-lg transition-transform transform hover:scale-105 text-white ${cargando ? 'bg-gray-400' : 'bg-red-600 hover:bg-red-700'}`}>
